...
declare file_name=$current_dir/words.csv
...
display_mission() {
    ...
    ###  set iteration  ###
    line_count=$(grep -vE '^[[:space:]]*$' $mission | wc -l)
...
read_specific_line() {
    ...
    local line_content=$(sed -n "${line_number}p" "$file")
...
play_game() {
    ...
    ###  set iteration  ###
    line_count=$(grep -vE '^[[:space:]]*$' $file_name | wc -l)
